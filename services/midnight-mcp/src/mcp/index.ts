...
export class WalletServiceMCP {
   ...
   */
  public async getTransactionStatus(transactionId: string): Promise<TransactionStatusResult | null> {
    if (!this.isReady()) {
      throw new WalletServiceError(WalletServiceErrorType.WALLET_NOT_READY, 'Wallet is not ready');
    }
    
    try {
      const status = await this.wallet.getTransactionStatus(transactionId);
  ...
  public async getTransactions(): Promise<TransactionRecord[]> {
    ...
    try {
      return await this.wallet.getTransactions();
    } catch (error) {
  ...
  public async getPendingTransactions(): Promise<TransactionRecord[]> {
    ...
    try {
      return await this.wallet.getPendingTransactions();
    } catch (error) {
  ...
  public async getTokenBalance(tokenName: string): Promise<string> {
    return await this.wallet.getTokenBalance(tokenName);
  }
  ...
  /**
   * Get the wallet providers for contract deployment
   */
  public async getProviders(): Promise<any> {
    ...
    // Use the configureProviders function from marketplace API
    const { configureProviders } = await import('../integrations/marketplace/api.js');
    // Get the wallet instance from WalletManager
    const walletInstance = this.wallet.getWalletForProviders();
    if (!walletInstance) {
      throw new WalletServiceError(
        WalletServiceErrorType.WALLET_NOT_READY,
        'Wallet instance is not available'
      );
    }
    return await configureProviders(walletInstance);
  }

  /**
   * Get the wallet provider for contract operations
   */
  public async getWalletProvider(): Promise<any> {
    const providers = await this.getProviders();
    return providers.walletProvider;
  }
}
