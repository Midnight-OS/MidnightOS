...
export class ShieldedTokenManager {
  ...
  constructor(walletManager: WalletManager) {
    ...
    this.registerTokensFromEnv().catch(error => {
      this.logger.error('Failed to auto-register tokens from environment:', error);
    });
  ...
  public async registerToken(
    ...
    try {
      ...
      await this.tokenRegistryDb.registerToken(tokenInfo);
   ...
   */
  public async getTokenInfo(tokenIdentifier: string): Promise<TokenInfo | null> {
    // First try to find by name (case insensitive)
    let tokenInfo = await this.tokenRegistryDb.getTokenByName(tokenIdentifier.toLowerCase());
    
    // If not found by name, try to find by symbol (case insensitive)
    if (!tokenInfo) {
      tokenInfo = await this.tokenRegistryDb.getTokenBySymbol(tokenIdentifier.toUpperCase());
    }
  ...
  public async listWalletTokens(): Promise<TokenBalance[]> {
    try {
      ...
      const registeredTokens = await this.tokenRegistryDb.getAllTokens();
  ...
  public async getRegisteredTokenNames(): Promise<string[]> {
    const allTokens = await this.tokenRegistryDb.getAllTokens();
  ...
  public async isTokenRegistered(tokenName: string): Promise<boolean> {
    return await this.tokenRegistryDb.isTokenRegistered(tokenName.toLowerCase());
  }
  ...
  public async unregisterToken(tokenName: string): Promise<boolean> {
    return await this.tokenRegistryDb.unregisterToken(tokenName.toLowerCase());
  }
  ...
  public async getRegistryStats(): Promise<{ totalTokens: number; tokensBySymbol: Record<string, number> }> {
    return await this.tokenRegistryDb.getRegistryStats();
  }
   ...
   */
  private async registerTokensFromEnv(): Promise<void> {
    try {
      ...
      if (tokenConfigs.length > 0) {
        ...
        const result = await this.registerTokensBatch(tokenConfigs);
   ...
   */
  public async registerTokensBatch(tokenConfigs: TokenConfig[]): Promise<BatchTokenRegistrationResult> {
    ...
    for (const config of tokenConfigs) {
      try {
        ...
        if (await this.isTokenRegistered(config.name)) {
   ...
   * @returns Batch registration result
   */
  public async registerTokensFromEnvString(envValue: string): Promise<BatchTokenRegistrationResult> {
    const tokenConfigs = parseTokensFromMultipleEnvVars();
    return await this.registerTokensBatch(tokenConfigs);
  }
