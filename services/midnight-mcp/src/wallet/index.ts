...
import { PrismaTransactionService } from './db/PrismaTransactionService.js';
...
export class WalletManager {
  ...
  private transactionDb: PrismaTransactionService;
  ...
  constructor(networkId: NetworkId, seed: string, walletFilename: string, externalConfig?: WalletConfig) {
    ...
    // Initialize the transaction database service
    this.transactionDb = new PrismaTransactionService();
    this.logger.info('Transaction database service initialized');
    ...
    this.walletSyncSubscription = this.wallet.state().subscribe({
      next: async (state) => {
        try {
          ...
          // Check if wallet is ready (has address and minimal sync)
          // For new wallets, having an address means it's ready for basic operations
          const isReady = this.walletAddress && this.walletAddress !== '' && 
                         (isSynced || (applyGap === 0n && sourceGap === 0n));
          
          if (isReady) {
            if (!this.ready) {
              this.ready = true;
              this.logger.info('Wallet is ready for operations!');
              ...
              this.logger.info(`Sync status - isSynced: ${isSynced}, applyGap: ${applyGap}, sourceGap: ${sourceGap}`);
  ...
  public async sendFunds(to: string, amount: string): Promise<SendFundsResult> {
    ...
    try {
      ...
      await this.transactionDb.markTransactionAsSent(transaction.id, submittedTransaction);
  ...
  public async close(): Promise<void> {
    try {
      ...
      // Prisma client cleanup is handled automatically
      this.logger.info('Transaction service cleanup completed');
  ...
  public async checkPendingTransactions(): Promise<void> {
    ...
    try {
      ...
      const sentTransactions = await this.transactionDb.getTransactionsByState(TransactionState.SENT);
      ...
      for (const tx of sentTransactions) {
        ...
        if (verificationResult.exists) {
          ...
          await this.transactionDb.markTransactionAsCompleted(tx.txIdentifier);
        } else {
  ...
  private async processSendFundsAsync(transactionId: string, to: string, amount: string, correlationId?: string): Promise<void> {
    ...
    try {
      ...
      await this.transactionDb.markTransactionAsSent(transactionId, submittedTransaction);
      ...
      await this.transactionDb.markTransactionAsFailed(transactionId, error instanceof Error ? `Failed at processing transaction: ${error.message}` : 'Unknown error processing transaction');
      throw error;
    }
  }
   ...
   */
  public async getTransactionStatus(id: string): Promise<TransactionStatusResult | null> {
    try {
      const transaction = await this.transactionDb.getTransactionById(id);
  ...
  public async registerToken(
    ...
    return await this.shieldedTokenManager.registerToken(name, symbol, contractAddress, domainSeparator, description, decimals);
  }
   ...
   */
  public async getTokenInfo(tokenName: string): Promise<TokenInfo | null> {
    return await this.shieldedTokenManager.getTokenInfo(tokenName);
  }
   ...
   */
  public async getTokenBalance(tokenName: string): Promise<string> {
    return await this.shieldedTokenManager.getTokenBalance(tokenName);
  }
   ...
   */
  public async listWalletTokens(): Promise<TokenBalance[]> {
    return await this.shieldedTokenManager.listWalletTokens();
  }
   ...
   */
  public async createCoinForVoting(tokenName: string, amount: bigint = 500n): Promise<CoinInfo> {
    return await this.shieldedTokenManager.createCoinForVoting(tokenName, amount);
  }
   ...
   * @returns Coin info for transactions
   */
  public async createCoinForFunding(tokenName: string, amount: bigint = 100n): Promise<CoinInfo> {
    return await this.shieldedTokenManager.createCoinForFunding(tokenName, amount);
  }
   ...
   */
  public async getRegisteredTokenNames(): Promise<string[]> {
    return await this.shieldedTokenManager.getRegisteredTokenNames();
  }
   ...
   */
  public async isTokenRegistered(tokenName: string): Promise<boolean> {
    return await this.shieldedTokenManager.isTokenRegistered(tokenName);
  }
   ...
   */
  public async unregisterToken(tokenName: string): Promise<boolean> {
    return await this.shieldedTokenManager.unregisterToken(tokenName);
  }
   ...
   */
  public async getTokenRegistryStats(): Promise<{ totalTokens: number; tokensBySymbol: Record<string, number> }> {
    return await this.shieldedTokenManager.getRegistryStats();
  }
   ...
   */
  public async registerTokensBatch(tokenConfigs: Array<{
    name: string;
    symbol: string;
    contractAddress: string;
    ...
    return await this.shieldedTokenManager.registerTokensBatch(tokenConfigs);
  }
   ...
   * @returns Batch registration result
   */
  public async registerTokensFromEnvString(envValue: string): Promise<{
    success: boolean;
    registeredCount: number;
    errors: Array<{ token: string; error: string }>;
    registeredTokens: string[];
  }> {
    return await this.shieldedTokenManager.registerTokensFromEnvString(envValue);
  }
  ...
  /**
   * Get wallet for provider configuration
   * Used for creating providers for contract interactions
   * @returns The wallet instance or null if not ready
   */
  public getWalletForProviders(): (Wallet & Resource) | null {
    return this.wallet;
  }
}
